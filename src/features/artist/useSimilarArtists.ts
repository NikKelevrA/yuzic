...
export function useSimilarArtists(input: SimilarArtistsInput) {
  ...
  // Pictures for the list only while artist artwork is allowed; part of the
  // key, so turning it on refetches with them.
  const withArtwork = useSourceUse(ARTIST_ARTWORK_USE)
  const queryKey = useMemo(
    () => [QueryKeys.ExploreSimilarArtists, input.mbid ?? input.name ?? '', input.limit ?? 8, withArtwork],
    [input.limit, input.mbid, input.name, withArtwork]
  )

  return useQuery({
    queryKey,
    queryFn: () => fetchSimilarArtistsFromScrobbles(input.name!, input.excludeName ?? undefined, input.limit ?? 8, { withArtwork }),
