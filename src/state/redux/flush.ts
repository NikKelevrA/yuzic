 ...
 *
 * Required lazily so importing this does not build the app store — hooks and
 * tests that bring their own store can use it without starting redux-persist.
 */
export function flushPersistedState(): Promise<void> {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const { persistor } = require('./store') as typeof import('./store');
