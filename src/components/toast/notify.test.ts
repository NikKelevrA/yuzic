import { notify, __getToasts, __resetToasts } from './notify';
...
describe('notify store', () => {
  ...
  it('adds a toast and returns a stable non-empty id', () => {
    ...
    expect(__getToasts()[0]).toMatchObject({ message: 'Saved', variant: 'success' });
  });
  ...
  it('reuses the id passed in options and updates in place (no stacking)', () => {
    ...
    expect(__getToasts()).toHaveLength(1);
    expect(__getToasts()[0]).toMatchObject({ message: 'Done', variant: 'success' });
  });
  ...
  it('applies finite defaults for success/error/info and Infinity for loading', () => {
    ...
    const byId = Object.fromEntries(__getToasts().map(t => [t.id, t.duration]));
  ...
  it('caps the visible stack at 3, dropping the oldest', () => {
    ...
    notify.info('3', { id: '3' });
    notify.info('4', { id: '4' });
    const ids = __getToasts().map(t => t.id);
  ...
  it('sorts oldest-first by sequence', () => {
    ...
    expect(__getToasts().map(t => t.message)).toEqual(['first', 'second']);
  });

  it('dismiss removes one id and is idempotent', () => {
    notify.error('Failed', { id: 'e1' });
    notify.dismiss('e1');
    expect(__getToasts()).toHaveLength(0);
  ...
  it('carries an action through to the stored toast', () => {
    ...
    expect(__getToasts()[0].action?.label).toBe('Undo');
    __getToasts()[0].action?.onPress();
